...
/**
 * MinoSwerveDriveSimulation simulates the behavior of a swerve drive train. It extends the
 ...
 * @param modules An array of MinoSwerveModule representing the swerve modules.
...
public class MinoSwerveDriveSimulation extends AbstractDriveTrainSimulation {
    ...
    public MinoSwerveDriveSimulation(DriveTrainSimulationConfig config, MinoSwerveModule[] modules, double cof, Pose2d initialPoseOnField) {
        super(config, initialPoseOnField);
        m_modules = modules;
        m_gyroSimulation = config.gyroSimulationFactory.get();
        m_maxFrictionForcePerModule = config.robotMass.div(m_modules.length).times(kG).times(cof).in(Newton);
    }
    ...
    public void simulationSubTick() {
        ...
        final Transform2d simRobotVelocity =
            new Transform2d(chassisSpeeds.vxMetersPerSecond, chassisSpeeds.vyMetersPerSecond, new Rotation2d(chassisSpeeds.omegaRadiansPerSecond));
        ...
        for (var module : m_modules) {
            ...
            final Translation2d rotVectorComponentAtModule = modulePosition.div(moduleDist).rotateBy(Rotation2d.kCCW_Pi_2).times(magnitude);
            final Translation2d totalVectorAtModule = simRobotVelocity.getTranslation().plus(rotVectorComponentAtModule);
            final double moduleAngle = module.getSteeringAngle();
            final double moduleAlignedVelocity = totalVectorAtModule.getNorm() < 1e-3
                ? 0.0
                : totalVectorAtModule.getNorm() * Math.cos(moduleAngle - totalVectorAtModule.getAngle().getRadians());
            ...
            final double moduleLateralVelocity = totalVectorAtModule.getNorm() < 1e-3
                ? 0.0
                : totalVectorAtModule.getNorm() * Math.sin(moduleAngle - totalVectorAtModule.getAngle().getRadians());

            Translation2d frictionForceVector = new Translation2d(1, new Rotation2d(moduleAngle)).rotateBy(Rotation2d.kCCW_Pi_2).times(
                m_maxFrictionForcePerModule * (Math.abs(moduleLateralVelocity) > 1e-2 ? Math.signum(moduleLateralVelocity) : 0.0));
            ...
            final Translation2d modulePositionWorldCoordinates = modulePosition.rotateBy(simPose.getRotation()).plus(simPose.getTranslation());
            final Translation2d moduleForceWorldCoordinates = moduleForce.rotateBy(simPose.getRotation());
            final Translation2d frictionForceVectorWorldCoordinates = frictionForceVector.rotateBy(simPose.getRotation());

            // Apply module forces.
            super.applyForce(
                new Vector2(moduleForceWorldCoordinates.getX(), moduleForceWorldCoordinates.getY()), 
                new Vector2(modulePositionWorldCoordinates.getX(), modulePositionWorldCoordinates.getY())
            );

            // Apply friction forces.
            super.applyForce(
                new Vector2(frictionForceVectorWorldCoordinates.getX(),frictionForceVectorWorldCoordinates.getY()),
                new Vector2(modulePositionWorldCoordinates.getX(), modulePositionWorldCoordinates.getY())
            );
        }
