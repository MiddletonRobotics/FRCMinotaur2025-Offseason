...
public class SwerveSetpointGenerator {
    ...
    public SwerveSetpointGenerator(final SwerveDriveKinematics kinematics, final Translation2d[] positions) {
        this.mKinematics = kinematics;
        this.mModules = positions;
    }
    ...
    private double findRoot(Function2d func, double x_0, double y_0, double f_0, double x_1, double y_1, double f_1, int iterations_left) {
        if (iterations_left < 0 || EqualsUtility.epsilonEquals(f_0, f_1)) {
            return 1.0;
        }
    ...
    public SwerveSetpoint generateSetpoint(final KinematicLimits limits, final SwerveSetpoint prevSetpoint, ChassisSpeeds desiredState, double dt) {
        ...
        if (EqualsUtility.epsilonEquals(desiredState.vxMetersPerSecond, 0.0) && EqualsUtility.epsilonEquals(desiredState.vyMetersPerSecond, 0.0) && EqualsUtility.epsilonEquals(desiredState.omegaRadiansPerSecond, 0.0)) {
            need_to_steer = false;
        ...
        double[] prev_vx = new double[mModules.length];
        double[] prev_vy = new double[mModules.length];
        Rotation2d[] prev_heading = new Rotation2d[mModules.length];
        double[] desired_vx = new double[mModules.length];
        double[] desired_vy = new double[mModules.length];
        Rotation2d[] desired_heading = new Rotation2d[mModules.length];
        ...
        for (int i = 0; i < mModules.length; ++i) {
            ...
            if (prevSetpoint.moduleStates[i].speedMetersPerSecond < 0.0) {
                prev_heading[i] = prev_heading[i].plus(new Rotation2d(Math.PI));
            }
            ...
            if (desiredModuleState[i].speedMetersPerSecond < 0.0) {
                desired_heading[i] = desired_heading[i].plus(new Rotation2d(Math.PI));
            }
        ...
        boolean prevZero = EqualsUtility.epsilonEquals(prevSetpoint.chassisSpeeds.vxMetersPerSecond, 0.0) && EqualsUtility.epsilonEquals(prevSetpoint.chassisSpeeds.vyMetersPerSecond, 0.0) && EqualsUtility.epsilonEquals(prevSetpoint.chassisSpeeds.omegaRadiansPerSecond, 0.0);
        boolean desiredZero = EqualsUtility.epsilonEquals(prevSetpoint.chassisSpeeds.vxMetersPerSecond, 0.0) && EqualsUtility.epsilonEquals(prevSetpoint.chassisSpeeds.vyMetersPerSecond, 0.0) && EqualsUtility.epsilonEquals(prevSetpoint.chassisSpeeds.omegaRadiansPerSecond, 0.0);
        if (all_modules_should_flip && !prevZero && !desiredZero) {
        ...
        List<Optional<Rotation2d>> overrideSteering = new ArrayList<>(mModules.length);
        ...
        for (int i = 0; i < mModules.length; ++i) {
            ...
            if (EqualsUtility.epsilonEquals(prevSetpoint.moduleStates[i].speedMetersPerSecond, 0.0)) {
                ...
                if (EqualsUtility.epsilonEquals(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
                ...
                if (numStepsNeeded <= 1.0) {
                    ...
                    overrideSteering.set(i, Optional.of(prevSetpoint.moduleStates[i].angle.rotateBy(new Rotation2d(Math.signum(necessaryRotation.getRadians()) * max_theta_step))));
                    min_s = 0.0;
                    continue;
                }
            }
            ...
            double s = findSteeringMaxS(prev_vx[i], prev_vy[i], prev_heading[i].getRadians(), desired_vx[i], desired_vy[i], desired_heading[i].getRadians(), max_theta_step, kMaxIterations);
        ...
        for (int i = 0; i < mModules.length; ++i) {
            ...
            double s = min_s * findDriveMaxS(prev_vx[i], prev_vy[i], Math.hypot(prev_vx[i], prev_vy[i]), vx_min_s, vy_min_s, Math.hypot(vx_min_s, vy_min_s), max_vel_step, kMaxIterations);
            min_s = Math.min(min_s, s);
        }

        ChassisSpeeds retSpeeds = new ChassisSpeeds(prevSetpoint.chassisSpeeds.vxMetersPerSecond + min_s * dx, prevSetpoint.chassisSpeeds.vyMetersPerSecond + min_s * dy, prevSetpoint.chassisSpeeds.omegaRadiansPerSecond + min_s * dtheta);
        ...
        for (int i = 0; i < mModules.length; ++i) {
            ...
            if (flipHeading(deltaRotation)) {
                retStates[i].angle = retStates[i].angle.plus(new Rotation2d(Math.PI));
                retStates[i].speedMetersPerSecond *= -1.0;
            }
        }
