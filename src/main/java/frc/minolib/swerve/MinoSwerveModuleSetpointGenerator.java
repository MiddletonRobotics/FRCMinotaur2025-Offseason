...
public class MinoSwerveModuleSetpointGenerator {
    ...
    public MinoSwerveModuleSetpointGenerator(final SwerveDriveKinematics kinematics, final double maxDriveSpeed, final double maxModuleAcceleration, final double maxModuleSteeringRate) {
     ...
     */
    public SwerveSetpoint getFeasibleSetpoint(final SwerveSetpoint previousSetpoint, final ChassisSpeeds desiredChassisSpeeds, final double allowedScrub) {
        final SwerveSetpoint desiredSetpoint = getDesaturatedSwerveSetpoint(desiredChassisSpeeds);
        ...
        SwerveSetpoint bestSetpoint = optimizeWithSlewRateLimit(getDesaturatedSwerveSetpoint(
            interpolateChassisSpeeds(previousSetpoint.chassisSpeeds, desiredSetpoint.chassisSpeeds, upperBound)).moduleStates,
        ...
        for (int n = 0; n < 10; n++) {
            ...
            final SwerveSetpoint testSetpoint = optimizeWithSlewRateLimit(getDesaturatedSwerveSetpoint(
                interpolateChassisSpeeds(previousSetpoint.chassisSpeeds, desiredSetpoint.chassisSpeeds, alpha)).moduleStates,
                previousSetpoint.moduleStates,
                m_maxSpeedDiff,
                m_maxAngleDiff
            );
    ...
    private static ChassisSpeeds interpolateChassisSpeeds(final ChassisSpeeds a, final ChassisSpeeds b, final double alpha) {
    ...
    private SwerveSetpoint optimizeWithSlewRateLimit(final SwerveModuleState[] targetStates, final SwerveModuleState[] currentStates, final double maxSpeedDiff, final double maxAngleDiff) {
        ...
        for (int i = 0; i < targetStates.length; i++) {
            ...
            final double speedDiff = optimizedStates[i].speedMetersPerSecond - currentStates[i].speedMetersPerSecond;
            if (speedDiff > maxSpeedDiff) {
                optimizedStates[i].speedMetersPerSecond = currentStates[i].speedMetersPerSecond + maxSpeedDiff;
            }
            ...
            final double angleDiff = optimizedStates[i].angle.getRadians() - currentStates[i].angle.getRadians();
            if (angleDiff > maxAngleDiff) {
                optimizedStates[i].angle = new Rotation2d(currentStates[i].angle.getRadians() + maxAngleDiff);
            }
    ...
    public double computeMaxModuleScrubs(final SwerveModuleState[] states) {
        ...
        for (int i = 0; i < states.length; i++) {
            final var cart1 = MathUtility.pol2cart(idealStates[i].speedMetersPerSecond, idealStates[i].angle.getRadians());
            final var cart2 = MathUtility.pol2cart(states[i].speedMetersPerSecond, states[i].angle.getRadians());
     ...
     */
    public static SwerveModuleState optimizeModule(final SwerveModuleState desiredState, final SwerveModuleState currentState) {
        double targetAngle = MathUtility.placeInScope(desiredState.angle.getRadians(), currentState.angle.getRadians());
