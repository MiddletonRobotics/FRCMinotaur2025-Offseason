...
public class MinoRobotOdometry {
    private static final MinoRobotOdometry robotOdometry = new MinoRobotOdometry();
    private SwerveDrivePoseEstimator estimator = null;
    private MinoPoseEstimator poseEstimator = null;
    ...
    private MinoRobotOdometry() {
        estimator = new SwerveDrivePoseEstimator(
            RobotConfiguration.getInstance().getSwerveDriveKinematics(),
            new Rotation2d(),
    ...
    public Pose2d getCustomEstimatedPose() {
        return this.poseEstimator != null ? this.poseEstimator.getCustomEstimatedPose() : new Pose2d();
    }
    ...
    public void resetPose(Rotation2d gyroAngle, SwerveModulePosition[] modulePositions, Pose2d poseMeters) {
        ...
        if (this.poseEstimator != null) this.poseEstimator.resetCustomPose(poseMeters);
    }
    ...
    public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds, double latencyAdjustmentSeconds, Matrix<N3, N1> visionMeasurementStdDevs) {
        ...
        if (INCLUDE_VISION_POSE_ESTIMATES) {
            this.estimator.addVisionMeasurement(visionRobotPoseMeters, adjustedTimestamp, visionMeasurementStdDevs);

            if (INCLUDE_VISION_POSE_ESTIMATES_IN_CUSTOM_ESTIMATOR && this.poseEstimator != null) {
                this.poseEstimator.addVisionMeasurement(visionRobotPoseMeters, Utils.fpgaToCurrentTime(adjustedTimestamp), visionMeasurementStdDevs);
            }
        }
     ...
     */
    public void setposeEstimator(MinoPoseEstimator customOdometry) {
        this.poseEstimator = customOdometry;
    }
     ...
     */
    public static MinoRobotOdometry getInstance() {
