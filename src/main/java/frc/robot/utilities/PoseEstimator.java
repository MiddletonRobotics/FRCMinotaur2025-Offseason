    ...
    public void addVisionData(List<TimestampedVisionUpdate> visionData) {
        for (var timestampedVisionUpdate : visionData) {
            ...
            if (updates.containsKey(timestamp)) {
                ...
                ArrayList<VisionUpdate> oldVisionUpdates = updates.get(timestamp).visionUpdates();
                ...
                Twist2d twist0 = GeometryUtility.multiply(nextUpdate.getValue().twist(), (timestamp - prevUpdate.getKey()) / (nextUpdate.getKey() - prevUpdate.getKey()));
                Twist2d twist1 = GeometryUtility.multiply(nextUpdate.getValue().twist(), (nextUpdate.getKey() - timestamp) / (nextUpdate.getKey() - prevUpdate.getKey()));

                // Add new pose updates
                ArrayList<VisionUpdate> newVisionUpdates = new ArrayList<VisionUpdate>();
