...
public class TeleopSwerve extends Command {
    ...
    private boolean driveFacingAngle;

    private static final double DEADBAND = 0.1;

    private final double maxVelocityMetersPerSecond = RobotConfiguration.getInstance().getRobotMaxVelocity().in(MetersPerSecond);
    private final double maxAngularVelocityRadiansPerSecond = RobotConfiguration.getInstance().getRobotMaxAngularVelocity().in(RadiansPerSecond);
    private final LoggedTunableNumber joystickPower = new LoggedTunableNumber("TeleopSwerve/joystickPower", 2.0);

    /**
     * Create a new TeleopSwerve command object.
     *
     * @param drivetrain the drivetrain subsystem instructed by this command
     * @param translationXSupplier the supplier of the translation x value as a percentage of the maximum velocity as defined by the standard field or robot coordinate system
     * @param translationYSupplier the supplier of the translation y value as a percentage of the maximum velocity as defined by the standard field or robot coordinate system
     * @param rotationSupplier the supplier of the rotation value as a percentage of the maximum rotational velocity as defined by the standard field or robot coordinate system
     */

    public TeleopSwerve(DrivetrainSubsystem drivetrain, DoubleSupplier translationXSupplier, DoubleSupplier translationYSupplier, DoubleSupplier rotationSupplier) {
    ...
    /**
     * Set the drive facing angle mode, which drives the robot to face a specific angle while translating based on the supplied values.
     * @param angleSupplier
     * @return
     */

    public TeleopSwerve setFacingAngle(Supplier<Rotation2d> angleSupplier) {
    ...
    public TeleopSwerve disableFacingAngle() {
        this.driveFacingAngle = false;
        return this;
    }

    public TeleopSwerve toggleFieldCentric() {
        if(drivetrain.getFieldRelative() == true) drivetrain.disableFieldRelative();
        else drivetrain.enableFieldRelative();
